...
let currentStream=null; let facing='user'; let currentDeviceId=null;
...
flipBtn.addEventListener('click',async()=>{ facing=facing==='user'?'environment':'user'; if(running) await startCamera(); });
cameraSelect.addEventListener('change',async()=>{ currentDeviceId=cameraSelect.value; if(running) await startCamera(); });

async function setupBackend(){ await tf.setBackend('webgl'); await tf.ready(); }
async function createDetector(){ const pd=window.poseDetection; if(!pd){ alert('pose-detection failed to load.'); throw new Error('poseDetection not available'); } const m=pd.SupportedModels.MoveNet; detector=await pd.createDetector(m,{modelType:'SinglePose.Thunder',enableSmoothing:true}); chipModel.innerHTML='Model<strong>Thunder</strong>'; }
async function startCamera(){
 const constraints={video:{width:640,height:480},audio:false};
 if(isMobile){ constraints.video.facingMode=facing; }
 else if(currentDeviceId){ constraints.video.deviceId={exact:currentDeviceId}; }
 ...
 if(isMobile){ flipBtn.style.display=vids.length>1?'inline-block':'none'; }
 else {
   cameraSelect.innerHTML='';
   vids.forEach((d,i)=>{ const opt=document.createElement('option'); opt.value=d.deviceId; opt.text=d.label||`Camera ${i+1}`; if(d.deviceId===currentDeviceId) opt.selected=true; cameraSelect.appendChild(opt); });
   cameraDropdown.style.display=vids.length>1?'flex':'none';
   if(!currentDeviceId && vids[0]) currentDeviceId=vids[0].deviceId;
 }
}
