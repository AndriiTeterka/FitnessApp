...
// Hide loading initially; only show while fetching the model
...
async function loadModel() {
    ...
    try {
        if (typeof tflite !== "undefined" && typeof tflite.setWasmPath === "function") {
        ...
        try {
            ...
            try {
                ...
                li.textContent = "Using TFJS GraphModel fallback";
                tipsList.appendChild(li);
            } catch (e2) {
                console.error('GraphModel fallback init failed', e2);
                const li = document.createElement("li");
                li.textContent = "Model failed to load. Running without tips.";
                tipsList.appendChild(li);
            }
        }
    } finally {
