...
async function createDetector(){
  ...
  if (!pd) {
    alert('pose-detection library failed to load. If your network blocks CDNs, use the server-side page (/) or allow CDN scripts.');
    throw new Error('poseDetection not available');
  }
  const m = pd.SupportedModels.MoveNet;
  const modelType = modelTypeSel.value === 'Thunder' ? 'SinglePose.Thunder' : 'SinglePose.Lightning';
  detector = await pd.createDetector(m, { modelType, enableSmoothing: true });
  chipModel.innerHTML = 'Model<strong>' + (modelTypeSel.value) + '</strong>';
}
...
function drawKeypointsAndSkeleton(kp, thr) {
  ...
  const pairs = [[5,7],[7,9],[6,8],[8,10],[11,13],[13,15],[12,14],[14,16],[5,6],[11,12],[5,11],[6,12]];
  ctx.fillStyle = '#66e0a3';
  for (let i = 0; i < kp.length; i++) {
    const {y,x,score} = kp[i];
    if ((score ?? 0) < thr) continue;
    ctx.beginPath(); ctx.arc(x, y, 5, 0, Math.PI*2); ctx.fill();
  }
  ctx.strokeStyle = '#7aa7ff'; ctx.lineWidth = 3;
  for (const [a,b] of pairs){
    if ((kp[a].score ?? 0) < thr || (kp[b].score ?? 0) < thr) continue;
...
function setTips(kp, thr){
  ...
  const s=(i)=> (kp[i].score ?? 0) >= thr;
  const LS=5, RS=6, LH=11, RH=12, LK=13, RK=14;
  if (s(LS)&&s(RS)&&s(LH)&&s(RH) && (s(LK)||s(RK))){
    const ms={ x:(kp[LS].x+kp[RS].x)/2, y:(kp[LS].y+kp[RS].y)/2 };
    const mh={ x:(kp[LH].x+kp[RH].x)/2, y:(kp[LH].y+kp[RH].y)/2 };
    const knee = s(LK)? kp[LK] : kp[RK];
    const torso = angleDeg(ms, mh, knee);
    if (torso != null && torso < 150) add('Keep chest up, reduce torso lean.');
  }
  // Optional: Knee drift cue
  if (s(LK) && s(LH) && Math.abs(kp[LK].x - kp[LH].x) > canvas.width * 0.2) add('Left knee drifting; align over foot.');
  if (s(RK) && s(RH) && Math.abs(kp[RK].x - kp[RH].x) > canvas.width * 0.2) add('Right knee drifting; align over foot.');
  if (!tipsList.children.length) add('Nice form!');
}

function angleDeg(a,b,c){
  const v1=[a.x-b.x, a.y-b.y]; const v2=[c.x-b.x, c.y-b.y];
...
async function loop(){
  ...
  try{
    ...
    if (poses && poses[0] && poses[0].keypoints) {
      // keypoints are in absolute pixel coords already
      drawKeypointsAndSkeleton(poses[0].keypoints, thr);
      setTips(poses[0].keypoints, thr);
      updatePoseScore(poses[0].keypoints);
    }
...
function updatePoseScore(kp){
  ...
  const scores = kp.map(p => p.score ?? 0);
